...
Но для сферической микросервисной системы в вакууме на сегодняшний день самым доступным решением для типовых задач мониторинга является Prometheus + Grafana. Это понятное и проверенное решение для проектов без экзотических требований по мониторингу.  
